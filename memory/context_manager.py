...
class ContextManager:
    ...
    async def sync_memory_to_db(
        ...
        # Extract and store invoice-related information in user preferences
        if "intent" in memory_entry.metadata and memory_entry.metadata["intent"] == "invoice_creator":
            await self.store_invoice_information(db, db_user_id, memory_entry)

        logger.info(f"Synced memory to database for conversation {conversation_id}")
        return True

    async def store_invoice_information(
        self, db: Session, user_id: UUID, memory_entry: MemoryEntry
    ) -> None:
        """
        Extract and store invoice information from conversation in user preferences.

        This helps build a profile of user information over time to reduce
        repetitive questions in future conversations.

        Args:
            db: Database session
            user_id: User ID
            memory_entry: Memory entry containing conversation data
        """
        # Fields we're interested in extracting from the conversation
        invoice_fields = {
            "company_name": ["company name", "business name", "your company", "vendor name"],
            "company_address": ["company address", "business address", "your address"],
            "company_phone": ["company phone", "business phone", "your phone"],
            "company_email": ["company email", "business email", "your email"],
            "client_name": ["client name", "customer name", "buyer name"],
            "client_company": ["client company", "customer company", "buyer company"],
            "client_address": ["client address", "customer address", "buyer address"],
            "client_phone": ["client phone", "customer phone", "buyer phone"],
            "client_email": ["client email", "customer email", "buyer email"],
            "payment_terms": ["payment terms", "payment due"],
        }

        # Extract invoice data from metadata if available
        extracted_data = {}
        if "extracted_invoice_data" in memory_entry.metadata:
            for field, value in memory_entry.metadata["extracted_invoice_data"].items():
                if field in invoice_fields and value:
                    extracted_data[field] = value

        # Extract any company/client information from recent user messages
        for message in memory_entry.messages:
            if message.get("role") == "user" and message.get("content"):
                content = message.get("content", "").lower()

                # Check each field pattern
                for field, patterns in invoice_fields.items():
                    if field in extracted_data:
                        continue  # Skip if already extracted from metadata

                    for pattern in patterns:
                        if pattern in content:
                            # Simple extraction logic - look for pattern: value format
                            # This is a basic implementation and could be enhanced
                            parts = content.split(pattern + ":")
                            if len(parts) > 1:
                                # Extract text after the pattern until punctuation or end
                                value = parts[1].split(".")[0].split(",")[0].strip()
                                if value:
                                    extracted_data[field] = value
                                    break

        # If we have any extracted data, update user preferences
        if extracted_data:
            try:
                # Get user
                user = db.query(User).filter(User.id == user_id).first()
                if not user:
                    logger.warning(f"User {user_id} not found when storing invoice information")
                    return
                ...
                for field, value in extracted_data.items():
                    if field not in preferences or not preferences[field]:
                        ...
                        logger.info(f"Stored {field}={value} in user preferences")
                ...
                logger.info(f"Updated user preferences with {len(extracted_data)} fields from conversation")
            except Exception as e:
                logger.error(f"Error storing invoice information: {e}", exc_info=True)
    ...
    async def load_state_with_history(
        ...
        conversation, created = await self.get_or_create_conversation(db, db_user_id)
        conv_id = str(conversation["id"] if isinstance(conversation, dict) else conversation.id)
        ...
        history_list = await self.get_conversation_history(db, conv_id)
