...
class LangGraphMemory:
    ...
    def __init__(
        ...
        try:
            memory_config = config.get("mongodb", key="memory", default={})
            self.max_memory_age = int(max_memory_age if max_memory_age is not None else os.environ.get(
                "MONGODB_MAX_MEMORY_AGE",
                memory_config.get("max_memory_age", 3600)
            ))
            self.max_messages = int(max_messages if max_messages is not None else os.environ.get(
                "MONGODB_MAX_MESSAGES",
                memory_config.get("max_messages", 50)
            ))
            self.message_window = int(message_window if message_window is not None else os.environ.get(
                "MONGODB_MESSAGE_WINDOW",
                memory_config.get("message_window", 10)
            ))
            if enable_context_window is None:
                enable_context_window_value = os.environ.get(
                    "MONGODB_ENABLE_CONTEXT_WINDOW",
                    memory_config.get("enable_context_window", "true")
                )
                self.enable_context_window = str(enable_context_window_value).lower() == "true"
            else:
                self.enable_context_window = enable_context_window

            if persist_memory is None:
                persist_memory_value = os.environ.get(
                    "MONGODB_PERSIST_MEMORY",
                    memory_config.get("persist_memory", "true")
                )
                self.persist_memory = str(persist_memory_value).lower() == "true"
            else:
        ...
        configured_use_mongodb = os.environ.get("USE_MONGODB", "false").lower() == "true"
        self.use_mongodb = self.persist_memory and (configured_use_mongodb if use_mongodb is None else use_mongodb)
        self.mongodb_memory = None
    ...
    def _get_mongodb_memory(self):
        if not self.use_mongodb:
            return None
        if self.mongodb_memory is not None:
            return self.mongodb_memory
        try:
            ...
            if getattr(self.mongodb_memory, "_fallback_mode", False):
                logger.warning("MongoDB memory initialized in fallback mode; using LangGraph in-memory storage")
                self.use_mongodb = False
                return None
            logger.info("Using MongoDB for LangGraph memory persistence")
            return self.mongodb_memory
        except Exception as e:
            logger.warning("MongoDB memory unavailable, falling back to in-memory storage: %s", str(e))
            self.use_mongodb = False
            return None
    ...
    def store(self, conversation_id: str, user_id: str, state: Any) -> None:
        ...
        if self.use_mongodb:
            try:
                mongodb_memory = self._get_mongodb_memory()
                if mongodb_memory:
                    mongodb_memory.store(conversation_id, user_id, state)
                    return
            except Exception as e:
    ...
    def retrieve(self, conversation_id: str) -> Optional[MemoryEntry]:
        ...
        if self.use_mongodb:
            try:
                mongodb_memory = self._get_mongodb_memory()
                memory_data = mongodb_memory.retrieve(conversation_id) if mongodb_memory else None
                if memory_data:
    ...
    def update_config(self,
        ...
        if persist_memory is not None:
            ...
            if prev_persist != persist_memory:
                self.use_mongodb = self.persist_memory and os.environ.get("USE_MONGODB", "false").lower() == "true"
                logger.info(f"Updated use_mongodb to {self.use_mongodb}")

        # Update MongoDB memory configuration if using it
        if self.use_mongodb and self.mongodb_memory is not None:
            try:
    ...
    def clear(self, conversation_id: str) -> bool:
        if self.use_mongodb:
            try:
                mongodb_memory = self._get_mongodb_memory()
                if mongodb_memory:
                    return mongodb_memory.clear(conversation_id)
            except Exception as e:
                logger.error(f"Error clearing MongoDB memory: {str(e)}", exc_info=True)
                self.use_mongodb = False
    ...
    def cleanup_expired(self) -> int:
        if self.use_mongodb:
            try:
                mongodb_memory = self._get_mongodb_memory()
                if mongodb_memory:
                    return mongodb_memory.cleanup_expired()
            except Exception as e:
                logger.error(f"Error cleaning MongoDB memory: {str(e)}", exc_info=True)
                self.use_mongodb = False

        now = time.time()
        expired_ids = [
            conversation_id
            for conversation_id, entry in self._memory.items()
            if now - entry.last_accessed > self.max_memory_age
        ]
    ...
    def get_active_conversations(self) -> List[str]:
        if self.use_mongodb:
            try:
                mongodb_memory = self._get_mongodb_memory()
                if mongodb_memory:
                    return mongodb_memory.get_active_conversations()
            except Exception as e:
                logger.error(f"Error listing MongoDB conversations: {str(e)}", exc_info=True)
                self.use_mongodb = False
    ...
    def get_user_conversations(self, user_id: str) -> List[str]:
        if self.use_mongodb:
            try:
                mongodb_memory = self._get_mongodb_memory()
                if mongodb_memory:
                    return mongodb_memory.get_user_conversations(user_id)
            except Exception as e:
                logger.error(f"Error listing MongoDB user conversations: {str(e)}", exc_info=True)
                self.use_mongodb = False
