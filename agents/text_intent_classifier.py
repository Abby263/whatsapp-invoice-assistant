...
class TextIntentClassifierAgent(BaseAgent):
    ...
    async def process(self,
        ...
        try:
            ...
            if (
                parsed_result["intent"] == IntentType.UNKNOWN
                and str(parsed_result.get("explanation", "")).startswith("Error:")
            ):
                parsed_result = self._heuristic_classify(user_input, formatted_history)
    ...
    def _heuristic_classify(self, text: str, history: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        text_lower = (text or "").lower()
        history_text = " ".join(str(message.get("content", "")) for message in (history or [])).lower()

        if any(word in text_lower for word in ["hello", "hi", "hey", "good morning", "good evening"]):
            return {"intent": IntentType.GREETING, "confidence": 0.9, "explanation": "Matched greeting keywords locally."}

        creation_terms = ["create", "generate", "make", "add a new", "new invoice", "new receipt"]
        if any(term in text_lower for term in creation_terms) and any(term in text_lower for term in ["invoice", "receipt"]):
            return {"intent": IntentType.INVOICE_CREATOR, "confidence": 0.9, "explanation": "Matched invoice creation keywords locally."}

        query_terms = ["invoice", "receipt", "spend", "spent", "cost", "price", "purchase", "bought", "vendor", "amazon", "total"]
        follow_up_terms = ["how much", "where", "what about", "last month", "it"]
        if any(term in text_lower for term in query_terms) or (
            any(term in text_lower for term in follow_up_terms)
            and any(term in history_text for term in query_terms)
        ):
            return {"intent": IntentType.INVOICE_QUERY, "confidence": 0.9, "explanation": "Matched invoice query keywords locally."}

        return {"intent": IntentType.GENERAL, "confidence": 0.85, "explanation": "No invoice action matched; treating as general text."}
