...
class InvoiceRAGAgent:
    ...
    async def _perform_vector_search(self, query_embedding: List[float], user_id: Union[str, int], db_session=None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"[WORKFLOW STEP] Using similarity threshold: {VECTOR_SIMILARITY_THRESHOLD} for vector search")
            ...
            # Log each result with its similarity score
            if invoices:
                logger.info(f"[WORKFLOW STEP] Vector search found {len(invoices)} results")
                for i, item in enumerate(invoices):
                    logger.info(f"[WORKFLOW STEP] Match {i+1}: '{item['description']}' - Similarity Score: {item['similarity']} (Threshold: {VECTOR_SIMILARITY_THRESHOLD})")
            
            # If no results found with strict threshold, try with a more relaxed threshold for diagnostic purposes
            if not invoices:
                logger.info(f"[WORKFLOW STEP] No results with threshold {VECTOR_SIMILARITY_THRESHOLD}, trying relaxed threshold 0.6 for diagnostic purposes")
                diagnostic_sql = f"""
                SELECT 
                    i.id as invoice_id,
                    i.invoice_number,
                ...
                # Execute diagnostic query to see what items would be found with a more relaxed threshold
                logger.info("[WORKFLOW STEP] Running diagnostic query to find most similar items regardless of threshold")
                diagnostic_result = db_session.execute(text(diagnostic_sql), {"user_id": user_id})
                ...
                if diagnostic_items:
                    logger.info(f"[WORKFLOW STEP] Diagnostic search found {len(diagnostic_items)} items:")
                    for i, item in enumerate(diagnostic_items):
                        logger.info(f"[WORKFLOW STEP] Item {i+1}: '{item['description']}' - Similarity: {item['similarity']}")
                else:
                    logger.info("[WORKFLOW STEP] Diagnostic search found no items at all - check if embeddings exist")
