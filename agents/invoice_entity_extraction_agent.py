...
class InvoiceEntityExtractionAgent(BaseAgent):
    ...
    async def process(self,
        ...
        try:
            ...
            try:
                ...
                if self._is_provider_error(parsed_result):
                    entities = self._heuristic_extract(user_text)
                    is_valid, validation_score = self._validate_extraction(
                        entities, has_context=bool(conversation_history)
                    )
                    return AgentOutput(
                        content=entities,
                        confidence=max(0.75, validation_score),
                        status="success" if is_valid else "incomplete_extraction",
                        metadata={
                            "original_text": user_text,
                            "explanation": "Extracted locally after LLM extraction failed.",
                            "raw_extraction_result": extraction_result,
                        },
                    )

                # LLMFactory may return either the entity map directly or wrap it
                # under an "entities" key with explanation/status metadata.
                if isinstance(parsed_result, dict):
                    if isinstance(parsed_result.get("entities"), dict):
                ...
                if "connection error" in str(extraction_result).lower() or str(extraction_result).lower().startswith("error"):
                    entities = self._heuristic_extract(user_text)
                    is_valid, validation_score = self._validate_extraction(
                        entities, has_context=bool(conversation_history)
                    )
    ...
    def _is_provider_error(self, parsed_result: Any) -> bool:
        ...
        explanation = str(parsed_result.get("explanation") or parsed_result.get("error") or "")
        lowered = explanation.lower()
        return any(
            marker in lowered
            for marker in (
                "live llm calls are disabled",
                "connection error",
                "api key",
                "rate limit",
                "timeout",
    ...
    def _heuristic_extract(self, text: str) -> Dict[str, Any]:
        ...
        vendor_match = re.search(r"(?:from|for)\s+([A-Z][A-Za-z0-9& .'-]+?)(?:\s+on|\s+for|\s+dated|$)", text)
        if vendor_match:
            vendor = vendor_match.group(1).strip(" .")
            if not vendor.startswith("$"):
                entities["vendor"] = vendor
        elif "amazon" in text_lower:
            entities["vendor"] = "Amazon"

        amount_match = re.search(r"\$\s*([0-9]+(?:\.[0-9]{1,2})?)|(?:total|amount)\s+\$?\s*([0-9]+(?:\.[0-9]{1,2})?)", text_lower)
        if amount_match:
            amount = float(next(group for group in amount_match.groups() if group))
            entities["total_amount"] = amount
            entities["currency"] = "USD"

        item_matches = re.findall(
            r"(?:(\d+(?:\.\d+)?)\s+)?([a-z][a-z ]+?)\s+(?:at|for)\s+\$?\s*([0-9]+(?:\.[0-9]{1,2})?)",
            text_lower,
        )
        items = []
        for quantity, description, price in item_matches:
            clean_description = description.strip(" ,:")
            if clean_description in {"invoice", "office supplies"}:
                continue
            qty = float(quantity or 1)
            unit_price = float(price)
            total_price = qty * unit_price if quantity else unit_price
            items.append({
        ...
        if items:
            ...
            entities.setdefault("total_amount", sum(item["total_price"] for item in items))
            entities.setdefault("currency", "USD")

        date_match = re.search(r"\b(?:on|dated)\s+([A-Za-z]+\s+\d{1,2})", text)
        if date_match:
            entities["invoice_date"] = date_match.group(1)

        return entities or {"description": text}
