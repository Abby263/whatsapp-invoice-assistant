...
class LLMFactory:
    ...
    async def agenerate_completion(
        ...
        # Generate completion based on provider
        if provider == ModelProvider.OPENAI:
            if os.environ.get("PYTEST_CURRENT_TEST") and not os.environ.get("ALLOW_LIVE_LLM_IN_TESTS"):
                raise RuntimeError("Live LLM calls are disabled during pytest")

            if AsyncOpenAI is None:
                logger.warning("AsyncOpenAI not available, running synchronous OpenAI call in a thread")
    ...
    async def classify_text_intent(self, input_text: str) -> str:
        ...
        try:
            ...
            response = await self.agenerate_completion(
                prompt=full_prompt,
    ...
    async def format_invoice_data(self, invoice_data: Union[str, Dict[str, Any]]) -> str:
        ...
        try:
            ...
            formatted_response = await self.agenerate_completion(prompt)
    ...
    async def validate_response(self,
        ...
        try:
            ...
            validation_result = await self.agenerate_completion(
                prompt=prompt,
                temperature=TemperatureSettings.VALIDATION,
                max_tokens=TokenLimits.MAX_OUTPUT_TOKENS_SHORT
            )
    ...
    async def text_to_sql(
        ...
        try:
            ...
            # Generate the SQL
            sql_result = await self.agenerate_completion(
                prompt=prompt,
