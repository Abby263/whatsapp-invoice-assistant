...
async def process_text_message(
    ...
    observability.log_event(
        logger, "text_message_started", sender=sender, user_id=user_id
    )
    ...
    try:
        if user_id is None:
            ...
            if user_id:
                ...
                logger.warning(
                    "Could not resolve text-message user_id from sender %s", sender
                )

        rate_limit = check_and_record(
            user_id,
            SCOPE_TEXT_TURN,
            request_id=whatsapp_message_sid
            or observability.current_context().get("request_id"),
...
async def process_file_message(
    ...
    try:
        rate_limit = check_and_record(
            user_id,
            SCOPE_MEDIA_UPLOAD,
            request_id=(file_metadata or {}).get("twilio_message_sid")
            or observability.current_context().get("request_id"),
        ...
        if isinstance(result, dict):
            ...
            if "invoice_id" in metadata:
                logger.info(
                    f"Successfully stored invoice with ID: {metadata['invoice_id']}"
                )
                if "item_ids" in metadata:
                    logger.info(f"Stored items: {len(metadata['item_ids'])} items")
            else:
                logger.warning(
                    "No invoice_id in result metadata, database storage may have failed"
                )
        ...
        observability.log_event(
            ...
            file_name=file_name,
            total_tokens=response_payload["metadata"]
            .get("token_usage", {})
            .get("total_tokens"),
        )
        return response_payload
...
async def process_whatsapp_message(message_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with observability.request_context(request_id=request_id, message_sid=message_sid):
        ...
        try:
            return await _process_claimed_whatsapp_message(
                message_data, message_sid, allow_queue=True
            )
        except Exception as e:
            logger.exception(f"Error processing WhatsApp message: {str(e)}")
            _mark_webhook_event_failed(message_sid, str(e))
            return {"status": "error", "message": GENERAL_FALLBACKS["no_response"]}


async def process_queued_whatsapp_message(
    ...
    with observability.request_context(request_id=request_id, message_sid=message_sid):
        return await _process_claimed_whatsapp_message(
            message_data, message_sid, allow_queue=False
        )
    ...
    try:
        ...
        if has_media and allow_queue and queue_enabled_for_media_count(media_count):
            job = enqueue_job(
                ...
                user_id=user_id,
                idempotency_key=f"twilio:{message_sid}:media_batch"
                if message_sid
                else None,
            )
            result = {
        ...
        if has_text:
            ...
            try:
                ...
                async with httpx.AsyncClient() as client:
                    for index in range(media_count):
                        ...
                        media_content_type = message_data.get(
                        ...
                        file_name = _twilio_media_filename(
                            media_url, media_content_type, index
                        )
                        auth = _twilio_media_auth(media_url)
                        response = await client.get(
                            media_url, auth=auth, follow_redirects=True
                        )
                        if response.status_code != 200:
                            logger.error(
                                "Failed to download media %s: %s",
                                index,
                        ...
                        if checksum in seen_checksums:
                            logger.info(
                                "Skipping duplicate media in same WhatsApp message: index=%s",
                                index,
                            )
                            results.append(
                                {
                        ...
                        result = await process_file_message(
                            ...
                            file_metadata={
                                ...
                                "twilio_media_sid": message_data.get(
                                    f"MediaSid{index}"
                                ),
                ...
                saved_conversation_id = save_conversation_turn(
                    ...
                    assistant_message=combined_result.get("message")
                    or combined_result.get("content"),
                    whatsapp_message_sid=message_sid,
                )
                if saved_conversation_id is not None:
            ...
            result = {"status": "error", "message": GENERAL_FALLBACKS["no_response"]}
            _mark_webhook_event_processed(message_sid, result)
            return result
    ...
    try:
        ...
        try:
            ...
            return {
                ...
                if isinstance(existing.response_payload, dict)
                else None,
            }
        finally:
            session.close()
        ...
        logger.warning(
            "Could not claim webhook event %s; continuing without replay protection: %s",
            message_sid,
            exc,
        )
...
def _mark_webhook_event_processed(
    message_sid: Optional[str], result: Dict[str, Any]
) -> None:
    _update_webhook_event(
        message_sid, "processed", response_payload=_webhook_response_payload(result)
    )
...
def _update_webhook_event(
    ...
    *,
    ...
    try:
        ...
        logger.warning(
            "Could not update webhook event %s to %s: %s", message_sid, status, exc
        )
...
def _webhook_response_payload(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "suppress_twiml_response": bool(
            result.get("suppress_twiml_response")
            or metadata.get("twilio_final_reply_sent")
        ),
        "metadata": {
...
def _duplicate_webhook_response(
    message_sid: Optional[str], claim: Dict[str, Any]
) -> Dict[str, Any]:
...
def _twilio_media_filename(media_url: str, media_content_type: str, index: int) -> str:
    file_name = (
        os.path.basename((media_url or "").split("?", 1)[0]) or f"receipt_{index + 1}"
    )
    return _ensure_media_filename_extension(file_name, media_content_type, index)


def _ensure_media_filename_extension(
    file_name: str, content_type: str, index: int
) -> str:
    ...
    guessed_ext = MEDIA_CONTENT_TYPE_EXTENSIONS.get(
...
def _combine_media_results(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    successes = [
        ...
        if _media_result_state(result) not in {"failed", "rejected"}
    ]
    errors = [
        result
        for result in results
        if _media_result_state(result) in {"failed", "rejected"}
    ]
    duplicates = [
        result for result in results if _media_result_state(result) == "duplicate"
    ]
    stored = [result for result in results if _media_result_state(result) == "saved"]
    pending = [
        result
        for result in results
...
def _media_result_summary_line(result: Dict[str, Any]) -> str:
    ...
    invoice_data = (
        metadata.get("invoice_data")
        if isinstance(metadata.get("invoice_data"), dict)
        else {}
    )
    ...
    if invoice_data:
        additional_info = (
            invoice_data.get("additional_info", {})
            if isinstance(invoice_data.get("additional_info"), dict)
            else {}
        )
        transaction = (
            invoice_data.get("transaction", {})
            if isinstance(invoice_data.get("transaction"), dict)
            else {}
        )
        financial = (
            invoice_data.get("financial", {})
            if isinstance(invoice_data.get("financial"), dict)
            else {}
        )
        document_type = _business_summary_label(
            ...
            or (
                "handwritten_ledger"
                if is_ledger_document(invoice_data)
        ...
        currency = (
            ...
            or ("INR" if is_ledger_document(invoice_data) else "USD")
        )
        amount = (
            financial.get("total")
        ...
        total = (
            _format_summary_money(amount, currency)
            if amount not in (None, "")
            else "Not visible"
        )
        items = (
            invoice_data.get("items")
            if isinstance(invoice_data.get("items"), list)
            else []
        )
    ...
    if state in {"failed", "rejected"}:
        reason = (
            ...
            or "Could not process this attachment"
        )
    ...
    if state == "awaiting approval":
        ...
        if approval_command:
            lines.append(f"   Save: {approval_command}")
        if rejection_command:
            lines.append(f"   Discard: {rejection_command}")
    return "\n".join(lines)
