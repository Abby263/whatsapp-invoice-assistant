...
async def find_similar_items(query: str, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not query_embedding:
            logger.error("Unable to generate embedding for similarity query")
            return []
        ...
        SELECT
            ...
            inv.vendor,
            inv.invoice_date,
            1 - (i.description_embedding <=> CAST(:query_embedding AS vector)) as similarity
        FROM
            items i
        JOIN
        ...
        ORDER BY
            i.description_embedding <=> CAST(:query_embedding AS vector)
        LIMIT :limit
        """)

        results = session.execute(sql, {"query_embedding": embedding_str, "limit": limit})
