...
@pytest.mark.asyncio
async def test_process_text_message_ignores_untrusted_supplied_history_for_user(
    monkeypatch,
):
    captured = {}
    ...
    await api.process_text_message(
        ...
        conversation_history=[
            {"role": "user", "content": "Other user private context"}
        ],
    )
...
@pytest.mark.asyncio
async def test_process_file_message_ignores_untrusted_supplied_history_for_user(
    ...
    await api.process_file_message(
        ...
        user_id="11",
        conversation_history=[
            {"role": "assistant", "content": "Other user file context"}
        ],
    )
...
@pytest.mark.asyncio
async def test_process_whatsapp_message_returns_help_command_without_user_lookup(
    monkeypatch,
):
...
async def test_process_whatsapp_text_sends_ack_before_slow_workflow(monkeypatch):
    ...
    async def fake_classify_intent(text, conversation_history=None):
        events.append("router")
        assert conversation_history == []
        return api.IntentType.INVOICE_QUERY.value

    async def fake_process_text_message(*args, **kwargs):
        events.append("process_text")
        assert kwargs["preclassified_intent"] == api.IntentType.INVOICE_QUERY.value
        return {
    ...
    monkeypatch.setattr(api, "classify_intent", fake_classify_intent)
...
@pytest.mark.asyncio
async def test_process_whatsapp_text_router_skips_ack_for_natural_greeting(monkeypatch):
    events = []
    acknowledgements = []
    final_messages = []
    ...
    async def fake_classify_intent(text, conversation_history=None):
        ...
        return api.IntentType.GREETING.value
    ...
    async def fake_process_text_message(*args, **kwargs):
        events.append("process_text")
        assert kwargs["preclassified_intent"] == api.IntentType.GREETING.value
        return {
            "message": "I'm doing well and ready to help.",
            "metadata": {"intent": "greeting", "source": "deterministic_greeting"},
            "status": "success",
            "type": "text",
            "user_id": "42",
        }
    ...
    result = await api.process_whatsapp_message(
        {
            ...
            "Body": "How are you?",
            "NumMedia": "0",
            "MessageSid": "SM_TEXT_GREETING",
        }
    )

    assert events == ["router", "history", "process_text"]
    assert acknowledgements == []
    assert final_messages == []
    assert result["message"] == "I'm doing well and ready to help."
...
async def test_process_whatsapp_text_queues_after_ack_when_async_enabled(monkeypatch):
    ...
    assert events == ["router", "ack", "enqueue"]
    ...
    assert (
        queued_jobs[0]["payload"]["RoutedIntent"] == api.IntentType.INVOICE_QUERY.value
    )
