...
def update_company_profile():
    ...
    try:
        ...
        # Get request data
        data = request.json
        user_id = linked_user.id if linked_user else data.get('user_id')
...
@app.route('/api/users/company-profile', methods=['GET'])
@app.route('/api/users/company-profile/<user_id>', methods=['GET'])
def get_company_profile(user_id=None):
...
@app.route('/api/generated-invoices', methods=['GET'])
def get_generated_invoices():
    """List outgoing invoices generated by the active user."""
    try:
        auth_context, linked_user, auth_response = require_linked_user()
        if auth_response:
            return auth_response
        ...
        invoices = list_generated_invoices(int(user_id))
        stats = get_generated_invoice_stats(int(user_id))
        return jsonify({
            'status': 'success',
        ...
        logger.exception(f"Error listing generated invoices: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': f"Error listing generated invoices: {str(e)}"
        }), 500


@app.route('/api/generated-invoices', methods=['POST'])
def create_generated_invoice():
    """Generate, store, and persist an outgoing invoice for the active user."""
    try:
        auth_context, linked_user, auth_response = require_linked_user()
        if auth_response:
            return auth_response
        ...
        data = request.json or {}
        user_id = linked_user.id if linked_user else data.get('user_id', USER_ID)
        if not user_id:
            return jsonify({
                'status': 'error',
                'message': 'User ID is required'
        ...
        invoice = generate_and_persist_invoice(
            data,
            user_id=int(user_id),
...
@app.route('/api/generated-invoices/analytics', methods=['GET'])
def generated_invoice_analytics():
    """Return generated-invoice analytics for the active user."""
    try:
        auth_context, linked_user, auth_response = require_linked_user()
        if auth_response:
            return auth_response
        ...
        return jsonify({
            'status': 'success',
            'analytics': get_generated_invoice_stats(int(user_id)),
            'user_id': str(user_id)
        })
    except Exception as e:
        logger.exception(f"Error loading generated invoice analytics: {str(e)}")
        return jsonify({
            'status': 'error',
